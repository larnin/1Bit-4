...
namespace Noise
{
    public class Turbulence
    {
        ...
        public float Get(float x, Lerp.Operator lerp = Lerp.Operator.Square)
        {
            float value = m_perlin.Get(x, lerp);
            return MakeTurbulence(value);
        }

        public float Get(float x, float y, Lerp.Operator lerp = Lerp.Operator.Square)
        {
            float value = m_perlin.Get(x, y, lerp);
            return MakeTurbulence(value);
        }

        public float Get(float x, float y, float z, Lerp.Operator lerp = Lerp.Operator.Square)
        {
